...
Problem:  Robot Protection
Link:     https://open.kattis.com/problems/robotprotection
...
def convex_hull(points):
    ...
    last = hull.pop(-1)
    if clockwise_turn(hull[-1], last, start) != 0 or (len(hull) == 1 and not (last[0]==hull[0][0] and last[1]==hull[0][1])):
        hull.append(last)
    return hull
...
def testcase():
    ...
    for i in range(len(hull)):
        area += (hull[i][0] * hull[(i+1)%len(hull)][1] - hull[(i+1)%len(hull)][0] * hull[i][1]) # Convex polygon formula
    print(abs(area) / 2)
    return True
